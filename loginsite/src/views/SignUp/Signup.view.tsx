...
export default function SignInForm() {
    ...
    const onChange = debounce((e: any)=>{
        ...
        if(value.length >=6 && (value.match(weakReg)|| value.match(MediumReg) || value.match(StrongReg))) {
            const isWeak = value.match(weakReg);
            const isMedium = value.match(MediumReg);
            const isStrong = value.match(StrongReg);
            isWeak && setWeak(true);
            isWeak && isMedium && setMedium(true);
            isWeak && isMedium && isStrong && setStrong(true);
        } else {
            setMedium(false);
            setStrong(false);
            setWeak(false);
    ...
    return (
        <>
            <div className='flex flex-col gap-1' id='text-box'>
                <svg
                ...
                <div className='text text1 text-wrap'>
                ...
                <form id='login' className='flex flex-col peer'>
                    ...
                    <div>
                        ...
                        ></input>
                        <div className='mt-2 flex h-1 flex-row w-full gap-1'>
                            <div className={`${weak ? "bg-red-500 w-full" : "bg-gray-400"} w-full`}></div>
                            <div className={`${Medium ? "bg-amber-500 w-full": "bg-gray-400"} w-full`}></div>
                            <div className={`${Strong ? "bg-lime-500 w-full" : "bg-gray-400"} w-full`}></div>
                        </div>
                        <div>{weak && "Weak" && Medium && "Medium" && Strong && "Strong"}</div>
                        <p className='text-base  invisible peer-invalid:visible text-red-600 text-sm'>
                ...
                <Link to="/signin">Already have account? Click here to sign in</Link>
            </div>
        </>
    );
}
